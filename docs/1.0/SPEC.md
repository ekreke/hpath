...
- [x] **T25 Per-agent model/prompt defaults + per-run model in the replay header**
  Contract: `Run.model` (resolved model id actually used; empty for legacy rows) and `repeated AgentSettings agents` on `AppSettings` (each entry: `agent_id`, server-filled `role`, `model`, `prompt`) + `make proto`. Server: `SettingsDoc.agents: Record<agentId, {model?, prompt?}>` seeded for the built-ins, validated (model must reference a configured model; prompt trimmed and capped at `MAX_AGENT_PROMPT_CHARS`), and read per run; `RunAgentInput` gains `modelOverride`/`promptOverride`, the pipeline prefers the override over the definition default, appends the prompt as a distinct "Agent instructions (configured in Settings)" block after the rendered system prompt, and reports `AgentRunResult.model`; RunCase/ParsePRD resolve the agent's settings at run start and snapshot the model on the run row (migration `0009_runs_model`); `GetSettings` fills `agents` from the kernel's `AgentRegistry` (no hardcoded ids), mock mirrors it. Desktop: Settings → General gains a per-agent block (model Select with "use default", extra-prompt textarea); `RunDto.model` + replay header shows the model; Rust/TS/IPC types extended.
  *Verify: `make proto` + `@hpath/contract build` + `@hpath/server build`/`test` (settings per-agent validation/override, pipeline override + prompt injection, seeded/legacy run rows) + `vite build` + `cargo check`; manual: set distinct agent models/prompts in Settings, run a case and confirm the replay header names the model and the prompt lands in the agent transcript.*
  Scope note (2026-09): landed in one pass. Server suite green at 297 tests (292 pass / 5 skipped), including the new `agent-run-overrides` pipeline tests and settings per-agent validation cases; `vite build` and `cargo check` pass. Pending manual: the live real-mode run (needs an LLM key) and the Settings/replay UI walk under `tauri dev`.

- [x] **T26 Batch case execution (list multi-select)**
  Execute several approved cases in one action. Contract: `BatchRunCase(BatchRunCaseRequest) returns (stream BatchEvent)` with a `started` / child `Event` / `finished` / `done` oneof and a `concurrency` field (0 = server default 2, capped at `MAX_BATCH_CONCURRENCY` = 4). Server: the single-run execution core is extracted into `startRun` / `resolveRunTarget`, shared by `RunCase` and the batch handler — per-run isolation, evidence recording, frame hubs and the PauseRun/ResumeRun/CancelRun RPCs are unchanged. The batch handler validates every target up front (empty list INVALID_ARGUMENT, unknown env/case NOT_FOUND, non-APPROVED FAILED_PRECONDITION — before any run starts), schedules with a bounded-concurrency helper, multiplexes the child events into one stream and closes with the tally. A disconnecting batch client leaves the child runs executing server-side (same RunCase semantics). Mock parity: `simulateRun` gained an `onRunCreated` hook and `batchRunCase` runs the scripted streams concurrently. Desktop: `batch_run_cases` Tauri command + `BatchEventDto` forwarded on the `batch-run-event` channel; the cases list gains a checkbox column (APPROVED only, header select-all with indeterminate state), a batch bar (selection count, env, concurrency 1–4, run/stop, final tally), per-row live status in the last-run column, and opening a case mid-batch shows its live `RunPanel` from the buffered + appended events; the batch bar renders in the detail view too, so a batch can still be stopped there; stop cancels every running child through `control_run` and cancels queued children the moment their `started` event arrives (retrying once, since a just-launched run may not have registered its kernel controller yet).
  *Verify: `make test` green (302 tests / 297 pass / 5 skipped) including 5 new `real batchRunCase handler (T26)` cases (up-front validation with no run rows, runId multiplexing + tally, concurrency cap, failing child does not affect siblings + tally, duplicate ids deduped) and the extended mock smoke (`batchRunCase` started/finished/done + event attribution); `vite build` + `tsc --noEmit` + `cargo check` green.*
  Scope note (2026-09): implemented end to end across contract/server/mock/desktop; a follow-up review pass fixed the stop-queued-children gap, the never-shown tally (batch bar unmounted with the cleared selection) and the stale per-row batch status after settle. Pending manual: the `tauri dev` list walk (select -> run -> per-row status -> open mid-batch live panel -> stop) and a real-mode batch against demo-app (needs an LLM key; the scheduler/transport path is covered by the handler tests and mock smoke).
