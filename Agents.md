...
- `make real` — start real-mode server (SQLite persistence; served: the read path, manual case management (create/update/delete/review), project update/delete, RunCase via the execute-agent with evidence in the artifact store, GetRun, DownloadArtifact, settings, status chat + chat sessions; still UNIMPLEMENTED: ParsePRD).
