...
.DEFAULT_GOAL := help
.PHONY: help install proto build dist mock real dev run smoke test unit-test restart stop clean verify up down logs docker-clean
...
test: build unit-test mock ## Full local verification: build, unit tests, start mock, run smoke, stop
	@sleep 0.5
	@pnpm --filter @hpath/server smoke; status=$$?; \
	 $(MAKE) stop; exit $$status

unit-test: ## Run package unit tests (agent kernel, registries, schema, template)
